...
Args:
    ...
    external_force (float): Additional external force to apply (e.g., from mouse interaction).
...
def compute_next_state(state, action, external_force=0.0):
    # Unpack the state vector
    x, x_dot, theta, theta_dot = state

    # Determine the direction of the force based on the action
    cart_force = FORCE_MAG if action == 1 else -FORCE_MAG

    # Add external force (e.g., from mouse interaction)
    total_cart_force = cart_force + external_force

    # Apply external force as torque on the pole (mouse pushing the pole tip)
    # Convert horizontal mouse force to angular force on the pole
    pole_torque = external_force * 0.5  # Scale factor for pole interaction
    ...
    temp = (
        total_cart_force + POLEMASS_LENGTH * theta_dot**2 * sintheta
    ) / TOTAL_MASS  # Formula: a = (F + m * l * θ_dot^2 * sin(θ)) / M

    # Calculate angular acceleration of the pole (including external torque)
    theta_acc = (
        GRAVITY * sintheta - costheta * temp + pole_torque / (MASSPEND * LENGTH)
    ) / (
        LENGTH * (4.0 / 3.0 - MASSPEND * costheta**2 / TOTAL_MASS)
    )  # Formula: θ_acc = (g * sin(θ) - cos(θ) * a + τ) / (l * (4/3 - m * cos^2(θ) / M))
